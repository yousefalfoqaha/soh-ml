...
\begin{figure}[H]
	\centerline{\includegraphics[width=\columnwidth]{dataset_structure.pdf}}
...
MCUs 2 and 7 were held out for validation and testing, respectively, as their SoH range ($95.6$--$67.5\%$) falls within the training range ($97.4$--$42.3\%$), evaluating interpolation rather than extrapolation.
...
Within each temperature band, the estimator performs consistently across the SoH spectrum. $-20^{\circ}\text{C}$ spans $68.1$--$93.7\%$ at $R^2 = 0.83$ and $40^{\circ}\text{C}$ spans $66.8$--$95.1\%$ at $R^2 = 0.98$.
Across discharge protocols, (performance metrics) with each protocol spanning multiple temperature bands (Table~\ref{tab:phase_soh_summary}). Constant and HPPC achieve the highest fits ($R^2 = 0.97$ and $0.99$), and Pulse follows ($R^2 = 0.93$). WLTC has an $R^2$ of $0.10$, but this reflects $2$ cycles restricted to the Initial phase, giving a narrow SoH band ($92.1$--$95.3\%$) (Table~\ref{tab:phase_soh_summary}).
