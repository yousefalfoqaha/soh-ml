...
class WindowsDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        mcus: list[str],
        data_path: Path,
        ...
        self.windows: list[DischargeInstance] = []
        ...
        print(f"Loaded {len(self.windows)} instances from {len(mcus)} MCU(s).")

    def __len__(self):
        return len(self.windows)

    def __getitem__(self, i):
        instance = self.windows[i]
        ...
        conditions = torch.tensor(
            [instance.soh, ambient_temperature], dtype=torch.float32
        )
